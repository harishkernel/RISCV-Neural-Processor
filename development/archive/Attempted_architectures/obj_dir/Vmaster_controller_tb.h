...
#ifndef VERILATED_VMASTER_CONTROLLER_TB_H_
#define VERILATED_VMASTER_CONTROLLER_TB_H_  // guard

#include "verilated.h"

class Vmaster_controller_tb__Syms;
class Vmaster_controller_tb___024root;

// This class is the main interface to the Verilated model
class alignas(VL_CACHE_LINE_BYTES) Vmaster_controller_tb VL_NOT_FINAL : public VerilatedModel {
    friend class Vmaster_controller_tb__Syms;
  private:
    // Symbol table holding complete model state (owned by this class)
    Vmaster_controller_tb__Syms* const vlSymsp;
    // Evaluation loop
  ...
  public:
    ...
    Vmaster_controller_tb___024root* const rootp;
    ...
    explicit Vmaster_controller_tb(VerilatedContext* contextp, const char* name = "TOP");
    explicit Vmaster_controller_tb(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~Vmaster_controller_tb();
  private:
    VL_UNCOPYABLE(Vmaster_controller_tb);  ///< Copying not allowed
